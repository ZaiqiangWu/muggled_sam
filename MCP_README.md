...
Mac 文件须先上传，不能直接将 `/Users/...` 传给 Ubuntu。`mcp_transfer.py` 只依赖 Python 标准库和系统 `sftp`，在 Mac 执行，使用已经配置好的 SSH key/agent；可在 `~/.ssh/config` 的 `hpc` alias 设置 HostName、User 和 ProxyJump。首次使用先通过普通 SSH 验证 host key。上传目标目录必须已存在：

```sh
python mcp_transfer.py upload --host hpc \
  --local /Users/ME/Videos/input.mp4 --remote /home/USER/sam3-input
```

命令打印 UUID 命名的 Ubuntu 文件路径。大视频走 SFTP 二进制流，不放入 MCP JSON。`input_location="mac"` 会明确要求先上传；上传后使用 `input_location="ubuntu"`。`output_location="mac"` 表示后续打包下载，`output_dir` 始终是 Ubuntu staging 路径，服务不会声称已写入 Mac。

## 完整调用示例

以下是 Codex 调用 MCP tools 的参数示例，`JOB` 替换为返回的 job_id：

1. `segment_video`：

```json
{"video_path":"/home/USER/sam3-input/UPLOADED_UUID.mp4","text_prompt":"quilted jacket","output_location":"mac","top_k":8,"random_check_frames":4,"max_retry":2,"prompt_frames":[0],"candidate_rank":0}
```
...
6. 调用 `package_result({"job_id":"JOB"})` 得到 `archive_path`，然后在 Mac 执行：

```sh
python mcp_transfer.py download --host hpc \
  --remote /home/USER/sam3-output/JOB/result_UUID.tar.gz \
  --local /Users/ME/Downloads/sam3-result.tar.gz
mkdir /Users/ME/Downloads/sam3-result
tar -xzf /Users/ME/Downloads/sam3-result.tar.gz -C /Users/ME/Downloads/sam3-result
```

下载不覆盖已有本地文件；归档包含 `result/rgba/*.png`、`result/overlay.mp4`、`result/analysis.json`、`result/previews/` 和包含完整 retry/review 历史的 `job.json`。Ubuntu 输出无需下载。归档也允许导出未通过结果，返回值明确标出 `quality_passed`。
...
权重加载错误会阻止启动；任务错误有 traceback 日志以及带类型的 error 字段，失败输出不参与最佳选择。每个任务和 attempt 独立，输出不静默覆盖；中断重启后任务标为 failed，可在剩余预算内重试。成功版本保留用于比较/下载，不自动删除用户产物；失败的 RGBA 和临时打包/下载文件会清理。磁盘需求随帧数和重试次数增长，完成后由用户清理任务目录。服务为单用户可信 SSH 会话设计。
...
测试使用合成视频和替代 predictor 验证产物、QA、局部保留、重试/审核门槛与 MCP tool 注册。真实 SAM3 精度、CUDA 显存、SSH 网络传输须在部署环境验证。

已在独立临时环境通过 7 项 CPU 测试（Python 3.13、MCP 1.30.0、OpenCV 4.13），包括 Streamable HTTP 握手、MCP image 响应和归档内容。部署目标仍按原工程使用 Python 3.12；未执行真实 GPU/权重推理或跨机器 SFTP 实传。运行任务期间请保持输入视频不变。
